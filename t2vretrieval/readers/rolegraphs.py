...
class RoleGraphDataset(t2vretrieval.readers.mpdata.MPDataset):
    def __init__(self, name_file, attn_ft_files, word2int_file,
        ...
        if ref_caption_file is None:
            ...
            if is_test or not is_train:  # test or validation
                ...
                if is_test and dname == "epic":
                    ...
                    self.captions = pandas.read_csv("annotation/epic100RET/EPIC_100_retrieval_test_sentence.csv")[
                        'narration'].values
        ...
        if is_train:
            print(
                f"using a threshold of {threshold_pos} [0, 1] to distinguish positives (>= thr) from negatives (< thr)")
    ...
    def get_caption_outs(self, out, sent, graph):
        ...
        node_idx = 1
        out['verb_masks'] = np.zeros((self.num_verbs, self.max_words_in_sent), np.bool_)
        ...
        out['noun_masks'] = np.zeros((self.num_nouns, self.max_words_in_sent), np.bool_)
        for knode, vnode in graph_nodes.items():
...
class AugmentedRoleGraphDataset(RoleGraphDataset):
    ...
    def __init__(self, name_file, attn_ft_files, word2int_file,
        ...
        super().__init__(name_file, attn_ft_files, word2int_file,
                         max_words_in_sent, num_verbs, num_nouns, ref_caption_file, ref_graph_file,
                         max_attn_len=max_attn_len, load_video_first=load_video_first, is_train=is_train,
                         _logger=_logger,
        ...
        if is_train:
            ...
            if video_verb_similars is not None:
                            ...
                            if i2 > i:
                                for j2, sent2 in enumerate(self.ref_captions[name2]):
                                    ...
                                    if not isinstance(vc2, list):
                                        vc2 = [vc2]
                                    if test_fn(vid_verb_classes, vc2, vid_noun_classes,
                                               nc_str):  # len(set(vc2).intersection(vid_verb_classes)) > 0:
                                        self.video_verb_similars[i].add(i2)
                                        self.video_verb_similars[i2].add(i)
                                    if test_fn(vid_noun_classes, nc_str, vid_verb_classes,
                                               vc2):  # len(set(nc_str).intersection(vid_noun_classes)) > 0:
                ...
                print(
                    f"--- Loaded dictionaries for similar videos: {len(self.video_noun_similars)} nouns, {len(self.video_verb_similars)} verbs ---")

            if original_dataframe != "":
                parse = lambda s, t: s.replace("[", "").replace("]", "").replace("'", "").replace(" ", "").replace(t,
                                                                                                                   " ").split(
                    ",")
                self.parse_fn = parse
                ...
                print(
                    f"--- Loaded synonyms' lists: {len(self.noun_synonyms)} nouns, {len(self.verb_synonyms)} verbs ---")
    ...
    def mix_fn(self, vid1, vid1_att, vid2, vid2_att, alpha=1):
        ...
        if not isinstance(vid1_att, np.ndarray):
            vid1_att = np.array(vid1_att)
        if not isinstance(vid2_att, np.ndarray):
            vid2_att = np.array(vid2_att)

        max_len = max(vid1.shape[0], vid2.shape[0])
        combined_att = (vid1_att + vid2_att).clip(max=max_len)

        # print(vid1.shape, vid2.shape)  # (Len, Feats)

        # return v, (vid1_att + vid2_att).clip(max=max(vid1.shape[0], vid2.shape[0]))

        return v, combined_att
        ...
        if self.is_train and perform_aug and self.perform_vid_aug:
            ...
            if len(valid_videos) > 0:
                ...
                if self.load_video_first:
                    ...
                    first_attn_fts, first_attn_len = self.pad_or_trim_feature(first_attn_fts, self.max_attn_len,
                                                                              trim_type='select')
                    other_name = self.names[other_video]
                    other_attn_fts = self.load_attn_ft_by_name(other_name, self.attn_ft_files)
                    other_attn_fts, other_attn_len = self.pad_or_trim_feature(other_attn_fts, self.max_attn_len,
                                                                              trim_type='select')
...
class VidNoiseAugmentedRoleGraphDataset(AugmentedRoleGraphDataset):
    ...
    def __init__(self, name_file, attn_ft_files, word2int_file,
        ...
        super().__init__(name_file, attn_ft_files, word2int_file,
                         ...
                         dataset_file=dataset_file, is_test=is_test, dname=dname, rel_mat_path=rel_mat_path,
                         threshold_pos=threshold_pos,
                         verb_classes=verb_classes, noun_classes=noun_classes, video_verb_similars=video_verb_similars,
                         video_noun_similars=video_noun_similars,
                         aug_chance=aug_chance, original_dataframe=original_dataframe, fix_lambda=fix_lambda,
                         aug_chance_txt=aug_chance_txt)
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            ...
            if self.perform_txt_aug and perform_aug_txt:
                ...
                nc_str = self.original_dataframe[
                    self.original_dataframe["narration_id"].str.contains(name.replace(".mp4", ""))]
                noun_classes = nc_str[
                    "all_noun_classes"].values  # list of string! (each string->list of classes for that sentence)
                verb_class = nc_str["verb_class"].values
                if isinstance(noun_classes[0], str):
                    noun_classes = [list(map(int, nc.replace("[", "").replace("]", "").split(","))) for nc in
                                    noun_classes if nc != '[]']
                if isinstance(verb_class[0], str):
                    verb_class = [list(map(int, nc.replace("[", "").replace("]", "").split(","))) for nc in verb_class
                                  if nc != '[]']
                if coin_N_or_V == 0:
                    # print(noun_classes)
                    if len(noun_classes) > 0 and len(verb_class) > 0:
                        chosen_noun_classes = noun_classes[randint(0, len(noun_classes) - 1)]
                        alt_sent = self.pick_alternative_sent_noun(
                            chosen_noun_classes[randint(0, len(chosen_noun_classes) - 1)],
                            verb_class[randint(0, len(verb_class) - 1)])
                    elif len(noun_classes) > 0:
                        chosen_noun_classes = noun_classes[randint(0, len(noun_classes) - 1)]
                        alt_sent = self._pick_alternative_sent_lesser("noun", important_class=chosen_noun_classes[
                            randint(0, len(chosen_noun_classes) - 1)])
                    elif len(verb_class) > 0:
                        alt_sent = self._pick_alternative_sent_lesser("noun", shared_classes=verb_class[
                            randint(0, len(verb_class) - 1)])
                    else:
                        alt_sent = sent
                else:
                    # print(verb_class)
                    if len(verb_class) > 0 and len(noun_classes) > 0:
                        ...
                        alt_sent = self.pick_alternative_sent_verb(
                            chosen_verb_classes[randint(0, len(chosen_verb_classes) - 1)],
                            noun_classes[randint(0, len(noun_classes) - 1)])
                    elif len(verb_class) > 0:
                        chosen_verb_classes = verb_class[randint(0, len(verb_class) - 1)]
                        alt_sent = self._pick_alternative_sent_lesser("verb", important_class=chosen_verb_classes[
                            randint(0, len(chosen_verb_classes) - 1)])
                    elif len(noun_classes) > 0:
                        alt_sent = self._pick_alternative_sent_lesser("verb", shared_classes=noun_classes[
                            randint(0, len(noun_classes) - 1)])
                    else:
                        alt_sent = sent
                if alt_sent == "":
                    alt_sent = sent
