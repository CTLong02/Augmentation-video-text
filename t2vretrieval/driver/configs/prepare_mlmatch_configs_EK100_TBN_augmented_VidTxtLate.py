...
def prepare_match_model(root_dir):
    ...
    output_dir = os.path.join(res_dir, 'mlmatch',
                              'ek100_TBN_aug%s_VcT_Late_m%s.vis.%s%s.txt.%s.%d.loss.%s.af.%s.%d%s.glove.init.%dep' %
    ...
    path_cfg = {
        ...
        'video_verb_similars': os.path.join(anno_dir, "EK100_video_verb_similars_coherent_nouns.json"),
        # os.path.join(anno_dir, "EK100_video_verb_similars.json"),
        'video_noun_similars': os.path.join(anno_dir, "EK100_video_noun_similars_coherent_verbs.json"),
        # os.path.join(anno_dir, "EK100_video_noun_similars.json"),
        'original_dataframe': os.path.join(anno_dir, "EPIC_100_retrieval_train.csv"),
    }
    for setname in ['trn', 'val', 'tst']:
        ...
        if setname == 'trn':
            path_cfg['dataset_file'][setname] = os.path.join(anno_dir, 'epic100_{}.json'.format(
                'train' if setname == 'trn' else 'test'))
        path_cfg['ref_graph_file'][setname] = os.path.join(anno_dir, 'sent2rolegraph_augmented.json')
