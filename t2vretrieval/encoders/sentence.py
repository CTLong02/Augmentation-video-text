...
class SentEncoder(nn.Module):
    ...
    def forward(self, cap_ids, cap_lens, init_states=None, return_dense=False, return_all_word_embeddings=False):
        '''
        Args:
...
class SentAttnEncoder(SentEncoder):
    ...
    def forward(self, cap_ids, cap_lens, init_states=None, return_dense=False, return_all_word_embeddings=False):
        if return_all_word_embeddings:
            hiddens, word_embeddings = super().forward(cap_ids, cap_lens, init_states=init_states, return_dense=True,
        ...
        if return_dense:
            if return_all_word_embeddings:
                return hiddens, attn_scores, word_embeddings
            return hiddens, attn_scores
        else:
            if return_all_word_embeddings:
                return torch.sum(hiddens * attn_scores.unsqueeze(2), 1), word_embeddings
