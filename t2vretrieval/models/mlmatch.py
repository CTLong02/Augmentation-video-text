...
class RoleGraphMatchModel(t2vretrieval.models.globalmatch.GlobalMatchModel):
    # long: I think this is the constructor
    ...
    def forward_loss(self, batch_data, step=None):
        ...
        if threshold_pos < 1:
            ...
            if "video_verb_classes" in batch_data:
                ...
                sent_rel = get_relevances_multi_caption(video_verbs=video_verb_classes, video_nouns=video_noun_classes,
                                                        batch_verbs=verb_class, batch_nouns=noun_classes).to(
                    self.device)
                noun_rel = get_relevances_multi_caption(video_nouns=video_noun_classes,
                                                        batch_nouns=noun_classes).to(self.device)
                verb_rel = get_relevances_multi_caption(video_verbs=video_verb_classes,
                                                        batch_verbs=verb_class).to(self.device)

            else:
                sent_rel = get_relevances_single_caption(batch_verbs=verb_class, batch_nouns=noun_classes).to(
                    self.device)
                noun_rel = get_relevances_single_caption(batch_nouns=noun_classes).to(self.device)
                verb_rel = get_relevances_single_caption(batch_verbs=verb_class).to(self.device)
        ...
        def get_hard_neg_relevance(_scores, _computed_relevance, delta=0.2,
                                   threshold_pos=1.):  # -> only relevance, no 1-relevance
        ...
        with torch.no_grad():
            ...
            if threshold_pos >= 1:
                if "video_verb_classes" in batch_data:
                    ...
                    sent_rel = get_relevances_multi_caption(video_verbs=video_verb_classes,
                                                            video_nouns=video_noun_classes,
                                                            batch_verbs=verb_class, batch_nouns=noun_classes).to(
                        self.device)
                    noun_rel = get_relevances_multi_caption(video_nouns=video_noun_classes,
                                                            batch_nouns=noun_classes).to(self.device)
...
class HPRoleGraphMatchModel(RoleGraphMatchModel):
    ...
    def get_hard_neg_pos_relevance(self, _scores, _computed_relevance, delta=0.2,
                                   threshold_pos=1.):  # -> only relevance, no 1-relevance
        # here we want to compute (1) the indices of the hard-negatives, and (2) their relevance
    ...
    def forward_loss(self, batch_data, step=None):
        ...
        if "video_verb_classes" in batch_data:
            ...
            sent_rel = get_relevances_multi_caption(video_verbs=video_verb_classes, video_nouns=video_noun_classes,
                                                    batch_verbs=verb_class, batch_nouns=noun_classes).to(self.device)
            noun_rel = get_relevances_multi_caption(video_nouns=video_noun_classes,
                                                    batch_nouns=noun_classes).to(self.device)
            verb_rel = get_relevances_multi_caption(video_verbs=video_verb_classes,
                                                    batch_verbs=verb_class).to(self.device)
            ...
            sent_rel = get_relevances_single_caption(batch_verbs=verb_class, batch_nouns=noun_classes).to(self.device)
            noun_rel = get_relevances_single_caption(batch_nouns=noun_classes).to(self.device)
            verb_rel = get_relevances_single_caption(batch_verbs=verb_class).to(self.device)
        ...
        with torch.no_grad():
            ...
            for n, v in [("sentence", sent_rel), ("noun", noun_rel), ("verb", verb_rel)]:
                self.logger.add_scalar(f"train_other/{n}_positives_relevance", torch.mean(v.diag()).item(), step)
                if threshold_pos < 1:
                    hn_rel_tv, hn_rel_vt, hp_rel_tv, hp_rel_vt = self.get_hard_neg_pos_relevance(scores, v,
                                                                                                 delta=self.criterion.margin,
                                                                                                 threshold_pos=threshold_pos)
                else:
                    hn_rel_tv, hn_rel_vt, hp_rel_tv, hp_rel_vt = self.get_hard_neg_pos_relevance(scores, v,
                                                                                                 delta=self.criterion.margin)
